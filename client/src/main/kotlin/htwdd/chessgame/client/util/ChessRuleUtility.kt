    ...
    companion object {
        ...
        fun getValidDropFields(image: Element): HashSet<Pair<Int, Int>> {
            ...
            if (row != null && col != null) {
                when (type) {
                    PieceType.BISHOP.toString() -> bishop(row, col)
                    PieceType.KING.toString() -> king(row, col)
                    PieceType.KNIGHT.toString() -> knight(row, col)
                    PieceType.PAWN.toString() -> pawn(row, col, pieceColor)
                    PieceType.QUEEN.toString() -> queen(row, col)
                    PieceType.ROOK.toString() -> rook(row, col)
                }
            }
        ...
        private fun bishop(row: Int, col: Int) {
            //top
            if (row != 1) {
                //left
                if (col != 1) {
        ...
        private fun king(row: Int, col: Int) {
        ...
        private fun knight(row: Int, col: Int) {
        ...
        private fun queen(row: Int, col: Int) {
            bishop(row, col)
            rook(row, col)
        }

        private fun rook(row: Int, col: Int) {
            //horizontal left
            if (col != 1) {
